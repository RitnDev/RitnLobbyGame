---------------------------------------------------------------------------------------------
local ritnlib = {}
ritnlib.gui =         require(ritnmods.tp.defines.functions.gui)
ritnlib.styles =      require(ritnmods.tp.defines.functions.styles)
...
local GuiElement = {
  ...
  -- Bouton principale
  button_main = {
    name = ritnmods.tp.defines.name.gui.menu.button_main,
    sprite = ritnmods.tp.defines.name.sprite.portal,
    style = ritnmods.tp.defines.name.gui.styles.button_main,
  },
  ...
  -- Frame Menu
  frame_menu = {
    name = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.frame_menu,
    caption = ritnmods.tp.defines.name.caption.frame_menu.titre
  },

  flow_restart = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.flow_restart,
  flow_admin = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.flow_admin,
  
  button_restart = {
    name = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.button_restart,
    caption = ritnmods.tp.defines.name.caption.frame_menu.button_restart,
  },

  button_exclusion = {
    name = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.button_exclusion,
    caption = ritnmods.tp.defines.name.caption.frame_menu.button_exclusion,
  },

  label_admin = {
    name = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.label_admin,
    caption = ritnmods.tp.defines.name.caption.frame_menu.label_admin, 
  },

  button_tp = {
    name = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.button_tp,
    caption = ritnmods.tp.defines.name.caption.frame_menu.button_tp,
  },

  button_clean = {
    name = prefix_main_menu .. ritnmods.tp.defines.name.gui.menu.button_clean,
    caption = ritnmods.tp.defines.name.caption.frame_menu.button_clean,
  },

  -- Frame Surfaces
  frame_surfaces = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.menu.frame_surfaces,
  panel_main = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.panel_main,
  panel_tp = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.menu.panel_tp,
  panel_clean = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.menu.panel_clean,
  panel_exclusion = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.menu.panel_exclusion,
  SurfacesFlow = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.SurfacesFlow,
  Pane = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.Pane,
  list = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.list,
  panel_dialog = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.panel_dialog,
  
  button_close_surfaces = {
    name = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.button_close,
    caption = ritnmods.tp.defines.name.caption.frame_portal.button_close,
  },

  button_valid_surfaces = {
    name = prefix_surfaces_menu .. ritnmods.tp.defines.name.gui.button_valid,
    caption = ritnmods.tp.defines.name.caption.frame_portal.button_valid,
  },

  -- Frame Restart
  frame_restart = {
    name = prefix_restart .. ritnmods.tp.defines.name.gui.menu.frame_restart,
    caption = ritnmods.tp.defines.name.caption.frame_restart.titre
  },

  flow_label = prefix_restart .. ritnmods.tp.defines.name.gui.menu.flow_label,

  label_warning1 = {
    name = prefix_restart .. ritnmods.tp.defines.name.gui.menu.label_warning1,
    caption = ritnmods.tp.defines.name.caption.frame_restart.label_warning1, 
  },

  label_warning2 = {
    name = prefix_restart .. ritnmods.tp.defines.name.gui.menu.label_warning2,
    caption = ritnmods.tp.defines.name.caption.frame_restart.label_warning2, 
  },

  flow_dialog = prefix_restart .. ritnmods.tp.defines.name.gui.panel_dialog,

  button_close_restart = {
    name = prefix_restart .. ritnmods.tp.defines.name.gui.menu.button_cancel,
    caption = ritnmods.tp.defines.name.caption.frame_portal.button_back,
  },

  button_valid_restart = {
    name = prefix_restart .. ritnmods.tp.defines.name.gui.button_valid,
    caption = ritnmods.tp.defines.name.caption.frame_portal.button_valid,
  },
...
local function create_button_main(LuaPlayer)
  ...
  local setting_value = settings.get_player_settings(LuaPlayer)[ritnmods.tp.defines.name.settings.enable_main_button].value
  if setting_value then
    local main_button = ritnlib.gui.createSpriteButton(
      ...
      ritnmods.tp.defines.name.gui.styles.ritn_main_sprite_button
    )
  end
...
local function create_gui_menu(LuaPlayer)
  ...
  if global.tp.surfaces[LuaSurface.name] == nil then return end -- correctif 1.8.1
  if global.tp.surfaces[LuaSurface.name].name == nil then return end
  local finish = global.tp.surfaces[LuaSurface.name].finish
  ...
  if LuaPlayer.name ~= LuaSurface.name then 
    ...
    if #global.tp.surfaces[LuaSurface.name].origine > 1 then
  ...
  ritnlib.styles.ritn_flow_surfaces(content.SurfacesFlow_tp.style)
    ...
    local MySurface = LuaPlayer.surface.name
    if global.tp.surfaces[MySurface] then
      for _,player in pairs(global.tp.surfaces[MySurface].origine) do 
        if player ~= nil then
...
local function verifGUIopen(LuaPlayer)
  ...
  and LuaPlayer.cursor_stack.name == ritnmods.tp.defines.name.item.remote then
    return true
  end

  if LuaPlayer.gui.screen[ritnmods.tp.defines.name.gui.NamerMain] ~= nil then return true end
  if LuaPlayer.gui.screen[ritnmods.tp.defines.name.gui.main_portal] ~= nil then return true end
...
local function button_main_close(LuaPlayer)
  local top = modGui.get_button_flow(LuaPlayer)
  local button_main = top[ritnmods.tp.defines.name.gui.menu.button_main]
...
local function frame_restart_close(LuaPlayer)
  local center = LuaPlayer.gui.center
  local frame_restart = center[prefix_restart .. ritnmods.tp.defines.name.gui.menu.frame_restart]
