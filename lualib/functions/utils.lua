...
local function clean(player_name, LuaPlayer)
    ...
    if global.tp.surfaces[player_name] then
        if game.surfaces[player_name] then

            local result = false
            for _,player in pairs(game.players) do 
                if global.tp.players[player.name] then -- fix 2.0.9
                    if global.tp.players[player.name].origine == player_name 
            ...
            for i,player in pairs(global.tp.surfaces[player_name].origine) do 
                global.tp.players[player] = nil
            end
        ...
        global.tp.surfaces[player_name] = nil
...
local function restart(LuaPlayer)
    ...
    if global.tp.surfaces[surface] then
        if game.surfaces[surface] then 
            ...
            for _,player in pairs(game.players) do 
                if player.surface.name == surface then 
                    if global.tp.players[player.name] then  -- fix 2.0.21
                        if global.tp.players[player.name].origine ~= surface then
                            if player.connected == true then
                                result = true
                            end  
                        end
        ...
        global.tp.surfaces[surface] = nil
        global.tp.surface_value = global.tp.surface_value - 1
...
local function add_exception(player_name)
    ...
    if not global.tp.surfaces[player_name] then return result end
    if global.tp.surfaces[player_name].exception == true then return result end
    global.tp.surfaces[player_name].exception = true
    result = true
    return result
end
...
local function remove_exception(player_name)
    ...
    if not game.surfaces[player_name] then return result end
    if not global.tp.surfaces[player_name] then return result end
    if global.tp.surfaces[player_name].exception == false then return result end
    global.tp.surfaces[player_name].exception = false
    global.tp.surfaces[player_name].last_use = game.tick
    result = true
    return result
end
...
local function view_exception(LuaPlayer)
    ...
    for _,player in pairs(global.tp.surfaces) do
