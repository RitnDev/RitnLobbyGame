...
---
---------------------------------------------------------------------------------------------
local util =          require(ritnmods.tp.defines.mods.vanilla.lib.util)
local crash_site =    require(ritnmods.tp.defines.mods.vanilla.lib.CrashSite)
---------------------------------------------------------------------------------------------
local ritnlib = {}
ritnlib.utils =       require(ritnmods.tp.defines.functions.utils)
--ritnlib.seablock =    require(ritnmods.tp.defines.mods.seablock)
ritnlib.player =      require(ritnmods.tp.defines.functions.player)
ritnlib.inventory =   require(ritnmods.tp.defines.functions.inventory)
---------------------------------------------------------------------------------------------
local prefix_enemy = ritnmods.tp.defines.prefix.enemy
...
local function UpdateCeaseFires(LuaSurface)
  ...
  if global.tp.surfaces[LuaSurface.name] then 
    ...
    if global.tp.surfaces[LuaSurface.name].map_used then 
      -- Map active
...
local function create_portal(LuaSurface, position, player_name, raiseBuilt, createBuildEffectSmoke)
    ...
    local LuaEntity = LuaSurface.create_entity({
        name = ritnmods.tp.defines.name.entity.portal,
        position = position,
        force = player_name,
...
local function generateSurface(LuaSurface)
  ...
  if LuaSurface.name == "nauvis" then 
    if global.tp.surfaces["nauvis"] then return end
  end

  if not global.tp.surfaces[LuaSurface.name] then
    global.tp.surface_value = global.tp.surface_value + 1
  end

  global.tp.surfaces[LuaSurface.name] = {
    ...
    finish = settings.startup[ritnmods.tp.defines.name.settings.restart].value,
  }
end
...
local function addPlayer(LuaPlayer)
  ...
  global.tp.surfaces[LuaSurface.name].players[LuaPlayer.name] = {
    name = LuaPlayer.name,
    tp = true,
    tick = game.tick,
  }
  global.tp.surfaces[LuaSurface.name].map_used = ritnlib.utils.tableBusy(global.tp.surfaces[LuaSurface.name].players)
...
local function removePlayer(LuaPlayer, oldSurface)
    for i,player in pairs(global.tp.surfaces[oldSurface.name].players) do
      if global.tp.surfaces[oldSurface.name].players[i].name == LuaPlayer.name then 
        global.tp.surfaces[oldSurface.name].players[i] = nil
      end 
    end
    
    global.tp.surfaces[oldSurface.name].map_used = ritnlib.utils.tableBusy(global.tp.surfaces[oldSurface.name].players)
...
local function createLobby(LuaPlayer)
  local prefix_lobby = ritnmods.tp.defines.prefix.lobby
...
local function createSurface(LuaPlayer)
  ...
  if global.tp.surface_value-1 < global.settings.surfaceMax then 

        -- Suppression de toutes requêtes en cours sur le joueur
        if global.tp.requests[LuaPlayer.name] then
          global.tp.requests[LuaPlayer.name] = nil
        end
        ...
        global.tp.surfaces["nauvis"].exception = true
        global.tp.surfaces["nauvis"].map_used = true
        generateSurface(LuaSurface)
        global.tp.surfaces[LuaSurface.name].exception = LuaPlayer.admin
        global.tp.surfaces[LuaSurface.name].inventories[LuaPlayer.name] = ritnlib.inventory.init()
        table.insert(global.tp.surfaces[LuaSurface.name].origine, LuaPlayer.name)

        -- 1.8.0
        -- Enregistrement de la surface d'origine
        if not global.tp.players[LuaPlayer.name] then 
          global.tp.players[LuaPlayer.name] = {origine = LuaSurface.name}
        end
        local origine = global.tp.players[LuaPlayer.name].origine
