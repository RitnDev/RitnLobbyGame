...
local function pollution_by_surface(LuaSurface)
    if global.tp.surfaces[LuaSurface.name] then 
        if global.tp.surfaces[LuaSurface.name].pollution then                     
            local count = global.tp.surfaces[LuaSurface.name].pollution.count
            local pollution_total = LuaSurface.get_total_pollution()
            global.tp.surfaces[LuaSurface.name].pollution.last = global.tp.surfaces[LuaSurface.name].pollution.current
            global.tp.surfaces[LuaSurface.name].pollution.current = pollution_total
            local ecart = global.tp.surfaces[LuaSurface.name].pollution.current - global.tp.surfaces[LuaSurface.name].pollution.last
            if ecart > 0 then 
                global.tp.surfaces[LuaSurface.name].pollution.count = count + ecart
            end
        end
    end
end
...
local function evolution_by_surface(LuaSurface)
    if global.tp.surfaces[LuaSurface.name] then  
        if global.tp.surfaces[LuaSurface.name].pollution then                     
            ...
            local time = global.tp.surfaces[LuaSurface.name].time
